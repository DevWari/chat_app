...
const ChatScreen = (props) => {
   ...
   const onAttachmentIcon = () => {
    ...
    useEffect(() => {
      ...
      setMessages(chatDetail[0].messages)
    },[])
        ...
        <ScrollView style={styles.inverted} contentContainerStyle={styles.content}>
          {
            messages.map((chats)=>{
              ...
              const isSend =  chats.type === "SEND"
              const messageType = chats.messageType === 'MONEY'
              
              return(
                <View key={chats.id} style={[isSend ? styles.sentContainer : styles.receivedContainer,styles.inverted]}>
